...
class WalkerController extends BaseController
{
	...
	// Get Client Location
	public function get_clientlocation()
	{
			...
			if ($walker_data = $this->getWalkerData($walker_id, $token, $is_admin)) {
				...
				if (is_token_active($walker_data->token_expiry)) {
					...
					foreach ($requests as $request) {
						...
						$request_data['owner']['ownerid'] = $owner->id;
	...
	// 	$walker_id = Input::get('id');
	// 	$request_id = Input::get('request_id');
	// 	$accepted = (int)Input::get('accepted');
	...
	// 							}
	// 						}
	// 						$response_array = array('success' => true, 'responsetype' => "bbbb");
	// 						$response_code = 200;
	// 					} else {
	...
	public function respond_request()
	{
		...
		// else {
		// 	$response_array = array('success' => false, 'active' => 'active');
		// 	$response_code = 200;
		// }
		else {
			$is_admin = $this->isAdmin($token);
