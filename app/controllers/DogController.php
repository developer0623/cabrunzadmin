...
// Get Walk Location
		...
		if ($validator->fails()) {
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					...
					$settings = Settings::where('key', 'default_inactive_timeout')->first();
					$inactive_timeout = $settings->value;
					$inactive_timeout = $inactive_timeout*60;//convert to seconds
					$now = time();

					$query = "SELECT walker.id, walker.latitude, walker.longitude, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) as distance from walker where is_available = 1 and is_active = 1 and is_approved = 1 and ($now - last_seen) < $inactive_timeout and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance order by distance";
		...
		if ($validator->fails()) {
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					...
					// $query = "SELECT walker.*, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) as distance, ($now - last_seen) as inactive_time from walker where is_available = 1 and is_active = 1 and is_approved = 1 and (inactive_time < $inactive_timeout) and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance and walker.deleted_at IS NULL and walker.id IN($typestring) order by distance";


					// $query = "SELECT walker.*, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) as distance, ($now - last_seen) as inactive_time from walker where is_available = 1 and is_active = 1 and is_approved = 1 and (($now - last_seen) < $inactive_timeout) and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance and walker.deleted_at IS NULL and walker.id IN($typestring) order by distance";
	...
	public function create_request()
	{
		...
		$source_location =Input::get('source_address');
		$destination_location =Input::get('destination_address');
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if($request){
					...
					if (Input::has('type')) {
						...
						$request->destination_address = $destination_location;
						$request->source_address = $source_location;
						...
						$request->request_start_time = date("Y-m-d H:i:s");
						$request->instruction = Input::get('instruction');
						$request->destination_address =$destination_location;
						$request->source_address = $source_location;
						$request->save();
					...
					if ($walker) {
						...
						$msg_array['unique_id'] = 1;
						//first walker
						$msg_array['walker_id']=$first_walker_id;
						$msg_array['request_id'] = $request->id;
						$msg_array['time_left_to_respond'] = $time_left;
						...
						$create_request = Settings::where('key', 'create_request')->first();
						$create_request = $create_request->value;
					...
					Queue::push('SendSmsController', array('first_name' => $owner_data->first_name,'last_name'=>$owner_data->last_name,'request_id'=>$request->id,'phone'=>$owner_data->phone));
					// send email
		...
		DontcreateReq:
		$response_array = ['success' => false , 'error' => 'You already have a pending request. Would you like to Cancel the pending request?','error_code' => 406];
		$response_code = 200;
		$response = Response::json($response_array, $response_code);
		return $response;
			...
			if ($validator->fails()) {
				...
				if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
					...
					if (is_token_active($owner_data->token_expiry) || $is_admin) {
						...
						$response_array = array(
							'success' => true,
							'price_per_unit_distance' => currency_converted($setdistance_price->value),
							'price_per_unit_time' => currency_converted($settime_price->value),
					...
					if ($request = Requests::find($request_id)) {
						...
						if ($request->owner_id == $owner_data->id) {
							...
							if ($request->current_walker) {
								...
								$cancelled = Settings::where('key', 'user_request_cancelled')->first();
							...
							if ($request->current_walker != 0) {
								...
								if ($request->confirmed_walker != 0) {
									...
									if ($request->is_completed == 1) {
										...
										if($request->amount_by == 1) {
											$bill['admin_total'] = $request->total;
											$bill['provider_total'] = $request->sub_total;

										} elseif ($request->amount_by == 2) {
											$bill['admin_total'] = $request->sub_total;
											$bill['provider_total'] = $request->total;
										} else {
											$bill['admin_total'] = $request->total;
											$bill['provider_total'] = $request->sub_total;
										}
										...
										$bill_walker  = Settings::where('key', 'service_fee')->first();
										$bill['walker']['amount'] = $bill_walker->service_fee;
										$bill['payment_mode']=$request->payment_mode;
										...
										$bill["actual_total_promo"]=$actual_total_promo;

										
										// $bill['promo_discount'] = currency_converted($discount);

										/*$base_price  = Settings::where('key', 'base_price')->first();
										$base_price = $base_price->value;

										$price_per_unit_time  = Settings::where('key', 'price_per_unit_time')->first();
										$price_per_unit_time = $price_per_unit_time->value;

										$price_per_unit_distance  = Settings::where('key', 'price_per_unit_distance')->first();
										$price_per_unit_distance = $price_per_unit_distance->value;
					*/					
										...
										$bill['actual_total']= currency_converted($adding_three);
									}
	...
	public function schedule_request()
	{
		...
		Log::info('query msg' . print_r($query, true));
		...
		foreach ($results as $result) {
			...
			if ($result->diff >= $timeout) {
				...
				if (isset($request_meta->walker_id)) {
					...
					$request_data["owner"]["destination_address"] = $request->destination_address;
					$request_data["owner"]["source_address"]=$request->source_address;
					...
					$pattern = str_replace('%user%', $owner_data->first_name, $pattern);
					$pattern = str_replace('%user_mobile%', $owner_data->phone, $pattern);
					sms_notification(1, 'admin', $pattern);
	...
	public function check_promo_code()
	{
		...
		if ($validator->fails()) {
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					// check promo code
					$promohistory =PromoHistory::where('promo_code',$promo_code)->where('user_id',$owner_id)->first();
					if($promohistory){
						$response_array = array('success' => false, 'error' => 'Already used promo code', 'error_code' => 416);
						$response_code = 200;
						$response = Response::json($response_array, $response_code);
						return $response;
					}
					$check_code = PromoCodes::where('coupon_code', $promo_code)->first();
					if ($check_code != NULL) {
						if ($check_code->state == 1 && date('Y-m-d H:i:s', strtotime($check_code->expiry)) > date('Y-m-d H:i:s')) {
            ...
            if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
                ...
                if (is_token_active($owner_data->token_expiry) || $is_admin) {
                    ...
                    if($request)
                    {
                        ...
                        $cancel_time = Settings::where('key','cancel_deduct_time')->first();
                        $cancel_deduct_time= $cancel_time->value;

                        if($dif >= $cancel_deduct_time)
                        {
                            $tot = Settings::where('key','cancel_deduct_amount')->first();
                            $total = $tot->value;
                            ...
                            if( $payment_data )
                            {
                                ...
                                $request->ledger_payment = $request->total;
                                $request->save();
                        ...
                        $owner = Walker::find($request->confirmed_walker);
                        $request_data = array();
                        $request_data['owner'] = array();
                        $request_data['owner']['name'] = $owner->first_name . " " . $owner->last_name;
                        $request_data['owner']['picture'] = $owner->picture;
                    ...
                    {
                        $response_array = array('success' => false,'error'=> 'Request Not found');
                    }
    ...
    public function walk_payment_accept() {

		$token = Input::get('token');
		$owner_id = Input::get('id');
		$request_id = Input::get('request_id');
		$walker_amount = Input::get('sub_total');
		$default_total = Input::get('total');
		...
		if ($validator->fails()) {
			...
			if ($owner_data = $this->getOwnerData($owner_id, $token, $is_admin)) {
				...
				if (is_token_active($owner_data->token_expiry) || $is_admin) {
					...
					if($request)
					{
						$request->total = $walker_amount;
						$request->sub_total = $walker_amount;

						if($request->save())
						{

							$response_array = array('success'=>true , 'message'=>"The Walker amount is saved",'request'=>$request);
							$response_code = 200;
						} else {
							$response_array = array('success' => false,'error'=>"Error Occured during process......!!!!");
							$response_code = 200;
						}
	...
	public function schedule_request_day()
	{
		$time = date("Y-m-d H:i:s");
		$timezone_app = Config::get('app.timezone');
		date_default_timezone_set($timezone_app);
		...
		$query = "SELECT id,owner_id,current_walker,TIMESTAMPDIFF(DAY,request_start_time, '$time') as diff from request where is_walker_started = 1 and is_completed = 0 and is_cancelled = 0";
		//SELECT id,owner_id,current_walker,TIMESTAMPDIFF(DAY,request_start_time, '2015-10-30 08:40:22') as diff from request
		$results = DB::select(DB::raw($query));
		foreach ($results as $result) {
			...
			if ($result->diff > 1) {
				...
				send_notifications($request->user_id, "owner", $title, $message);

			}
		}
	}
	public function cron_friend(){
		...
		$query = "SELECT id,user_id,friend_id,TIMESTAMPDIFF(SECOND,created_at, '$time') as diffs from friend where status = 0";
		...
		foreach ($results as $result) {
			...
			$friend = Friend::where('id',$result->id)->first();
			//cancelled
			$friend->status = 2;
			$friend->save();

			$title = "Payment not accepted";
			/*$message ="Your friend doesnt accepted the payment so it cancelled";
			$message_friend ="Yoy didnt accepted the payment so it cancelled";*/
            ...
            $response_array1 = array(
                                    'success' => true,
                                    'message'=>'Youdidnt accepted the payment so it cancelled',
                                    'push_id'=>6
                                );
                         
            $message_friend = $response_array1;

			send_notifications($owner_id, "owner", $title, $message);
	
			send_notifications($friend_id, "owner", $title, $message_friend); 
//
			}else{
    ...
    {
        Log::info("Cron Staging niki");
        $requests = Requests::where('later',1)->where('is_cancelled',0)->where('is_started',0)->where('confirmed_walker',0)->get();
        foreach ($requests as $request) 
        {
           //Log::info("cron running");
           // $t = get_user_time($request->request_start_time,1);
            $t = $request->request_start_time;
            //Log::info('req id = '.print_r($request->id,true));
            //Log::info('user req time = '.print_r($t,true));
            $d = strtotime($t);
            $t1 = date("Y-m-d H:i:s");
            Log::info("request time: ".print_r($t,true));
            Log::info("request time strto: ".print_r($d,true));
            //Log::info('user sys time = '.print_r($t1,true));
            $d1 = strtotime($t1);
            Log::info("system time ".print_r($t1,true));
            Log::info("system time strto: ".print_r($d1,true));
            $dif = $d - $d1; //future time string - current time string
 
            if($dif > 0) //
            {
                Log::info("1st condition Diff =".$dif);

                $dif = $dif/60;
                $dif = (int) $dif; 
                Log::info($dif);
          
                    $settings = Settings::where('key', 'scheduling_time')->first();
                    $schedule_time_limit = $settings->value;
                
                if($dif < $schedule_time_limit) {
                    Log::info("2nd condition");
                    $request_service = RequestServices::where('request_id', $request->id)->first();
                    $type = $request_service->type;
                    $request_id = $request->id;
                    $latitude = $request->s_latitude;
                    $longitude = $request->s_longitude; 

                    $typequery = "SELECT distinct provider_id from walker_services where is_available = 1 AND type IN($type)";
                    $typewalkers = DB::select(DB::raw($typequery));
                    ...
                    if (empty($typewalkers)) {
                        //new code
                        $typequery = "SELECT distinct provider_id from walker_services where is_available = 1 AND type != 3";
                        $typewalkers = DB::select(DB::raw($typequery));
                        if(empty($typewalkers)){
						...
						$query1 = "SELECT walker.id, 1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) as distance from walker where is_available = 1 and is_active = 1 and is_approved = 1 and ($now - last_seen) < $inactive_timeout and (1.609344 * 3956 * acos( cos( radians('$latitude') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$longitude') ) + sin( radians('$latitude') ) * sin( radians(latitude) ) ) ) <= $distance and walker.id IN($typestring) order by distance";
                          Log::info($query1);

                        $walkers = DB::select(DB::raw($query1));
                        Log::info('walkers = '.print_r($walkers,true));
                        $walker_list = array();
                        ...
                        if ($walker) {
                            ...
                            $json_resp = file_get_contents('http://maps.googleapis.com/maps/api/distancematrix/json?origins=' . $latitude . ',' . $longitude . '&destinations=' . $walker->latitude . ',' . $walker->longitude);
                            $data = json_decode($json_resp);
                            Log::info('data = ' . print_r($data, true));

                            $distance = $data->rows[0]->elements[0]->distance->value * 0.000621371;
                            ...
                            $msg_array['request_id'] = $request->id;
                            $msg_array['time_left_to_respond'] = $time_left;
                            $type_name = ProviderType::where('id', Input::get('type'))->first();
                            if ($type_name) {
                                $msg_array['vehicle_type'] = $type_name->name;
                            } else {
                                $msg_array['vehicle_type'] = "";
                            }
                            ...
                            $request_data = array();
                            $request_data['owner'] = array();
                            $request_data['owner']['id'] = $owner->id;
                            $request_data['owner']['name'] = $owner->first_name . " " . $owner->last_name;
                            $request_data['owner']['picture'] = $owner->picture;
                            $request_data['owner']['phone'] = $owner->phone;
                            $request_data['owner']['distance'] = $distance . " miles";
                            $request_data['owner']['time'] = $time;
                            $request_data['owner']['address'] = $request->s_address;
                            $request_data['owner']['latitude'] = $request->s_latitude;
                            $request_data['owner']['longitude'] = $request->s_longitude;
                            
                            if (($request->instruction != NULL) || ($request->instruction != "")) {
                                $request_data['owner']['instruction'] = $request->instruction;
                            } else {
                                $request_data['owner']['instruction'] = "";
                            }
                            if ($request->D_latitude != NULL) {
                                $request_data['owner']['d_latitude'] = $request->D_latitude;
                                $request_data['owner']['d_longitude'] = $request->D_longitude;
    ...
    public function scheduling_recurring(){
    	$date = date("Y/m/d");
    	$week_day = date('w', strtotime($date));
    	$query = RecurringRequest::all();
    	foreach ($query as $q) {
    		 $array = explode(',', $q->day); 
                    foreach($array as $value) 
                    {
                    	if($week_day == $value){
                    		$request = new Requests;
    						$request->owner_id=$q->owner_id;
            				$request->latitude=$q->latitude;
            				$request->longitude=$longitude;
            				$request->type = $q->type;
            				$request->instruction=$instruction;
            				$request->payment_mode=$q->payment_mode;
            				$request->source_address=$source_address;
            				$request->d_longitude=$q->d_longitude;
            				$request->d_latitude=$q->d_latitude;
            				$request->destination_address=$q->destination_address;
            				$request->distance=$q->distance;
 							$request->later=1;
 							$request->recurring=$q->id;
 							$date = date("Y-m-d");
            				$request->request_start_time=$date." ".$q->start;
            				$request->save();
